...
var upload = multer({ storage: storage }).fields([{ name: 'img1' },{ name: 'img2' }])
...
router.post("/notlogin", (req, res) => {
  upload(req, res, (err) => {
    ...
    const result = spawn('python', [path.join(__dirname, '..', 'AI', './pilldetectModel.py'),obj.img1[0].path,obj.img2[0].path]);
